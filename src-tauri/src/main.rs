...
use new_era_syringe_pumps_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m NewEraSyringePumps`,
    // i.e, run the `src-tauri/python/NewEraSyringePumps/__main__.py`
    let py_script = PythonScript::Module("NewEraSyringePumps".into());
